...
var KTUnidad = function () {
    ...
    function matcherStartsWith(params, data) {
        const term = $.trim(params.term || '');
        if (term === '') return data;
        if (typeof data.text === 'undefined') return null;

        return data.text.toUpperCase().startsWith(term.toUpperCase()) ? data : null;
    }

    const initMarcaAndModelo = () => {
        $('#IdMarca').select2({
            matcher: matcherStartsWith,
            width: '100%',
        ...
        $('#IdMarca').on('change', function () {
            const marcaId = parseInt($(this).val());
        ...
        $('#IdModelo').on('change', function () {
            const IdModelo = parseInt($(this).val());
