let fnConsultaUsuarios = new function () {
    "use strict";
    var self = this,
        $tableUsuarios = $("#kt_table_unidades"),
        _dataTable = {};
    ...
    function _onRenderDataTable() {
        $('[data-kt-unidades-action').off("click");
        $('[data-kt-unidades-action="edit"]').on("click", _onEdit)
    }
    function _onEdit() {
        let $tdElement = $(this).parents("td");
        window.location = "../Comercial/DetalleUnidad?unidadId=" + $tdElement.data("itemid");
    }
...
const exportButtons = (table) => {
    const documentTitle = 'Reporte unidades';
