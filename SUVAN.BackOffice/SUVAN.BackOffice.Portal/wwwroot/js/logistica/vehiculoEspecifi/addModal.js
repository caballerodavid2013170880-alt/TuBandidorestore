const KTDetalle = function () {
    ...
    const initModalEvent = () => {
        ...
        $(document).on('click', '.eyeModal', function () {
            const idEspecificaciones = $(this).data('idespecificaciones');

            $.get(`/VehiculoEspecificaciones/ObtenerDetalleVehiculo?idEspecificaciones=${idEspecificaciones}`, function (data) {
                configurarTabla(data);
                $('#vehiculoTabs a[data-tab="dimensioncapacidad"]').tab('show');
                modal.show();
            });
        });
